...
>
> 所有资源链接直接指向原始公开 URL（war.gov / cloudfront），在浏览器中点击即可查看，无需 clone 仓库。如需离线归档，提供 `scripts/download_resources.py` 一键镜像 6.7 GB 完整资源到本地。
...
脚本会按 `download_manifest.json` 镜像所有资源到 `downloads/` 目录。如果你只想浏览文档与资源，**完全不需要 clone**——直接在 GitHub 上点开 md 文件，资源链接已全部指向原始公开 URL。
...
- **资源链接**：全部指向原始公开 URL，浏览器内直接打开；如需离线归档，跑下载脚本即可镜像。
